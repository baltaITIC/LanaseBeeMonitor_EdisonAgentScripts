...
class accelcomp:
    ...
    def __init__(self):
        ...
        if(whoami == 0x49):
            bus.write_byte_data(LSM303D_ADDR, CTRL_REG1, 0x57) # 0x57 = ODR=50hz, all accel axes on ## maybe 
0x27 is Low Res?
    ...
    def getTiltHeading(self):
        ...
        try:
            ...
            roll = math.asin(truncate[Y]/math.cos(pitch)) if abs(math.cos(pitch)) &gt;= abs(truncate[Y]) else 
0
            ...
            print &quot;Pitch {}, cos(pitch) {}, Bool(cos(pitch)) {}&quot;.format(pitch, math.cos(pitch), 
bool(math.cos(pitch)))
...
if __name__ == &#039;__main__&#039;:
    ...
    while True:
        ...
        lsm.getHeading()
        lsm.getTiltHeading()
        print &#039;{0:.3f} |{1:.3f} |{2:.3f} ||{3:6} |{4:6} |{5:6} ||  {6:6} deg     |  {7:6} deg     
|&#039;.format(
                    lsm.accel[X], lsm.accel[Y], lsm.accel[Z], lsm.mag[X], lsm.mag[Y], lsm.mag[Z], 
